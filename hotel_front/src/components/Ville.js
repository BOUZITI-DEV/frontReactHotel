...
export function Ville() {
    ...
    const displayAll = () => {
        fetch('http://localhost:8888/SERVICE-CHAMBRE/api/ville/findAll')
    ...
    const countVilles = () => {
        fetch('http://localhost:8888/SERVICE-CHAMBRE/api/ville/count')
            .then(response => response.json())
    ...
    const addNewVille = () => {
        fetch('http://localhost:8888/SERVICE-CHAMBRE/api/ville/save', {
            method: 'POST',
            headers: {
    ...
    const deleteVille = (id) => {
        fetch(`http://localhost:8888/SERVICE-CHAMBRE/api/ville/delete/${id}`, {
            method: 'DELETE'
        })
            .then((response) => {
                if (response.ok) {
    ...
    const patchVille = () => {
        fetch('http://localhost:8888/SERVICE-CHAMBRE/api/ville/put', {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json'
            },
                ...
                {villes.map((ville) => (
                  <div key={ville.id} className="card bg-base-100 shadow-xl">
                    <div className="card-body">
                      <h2 className="card-title text-lg">{ville?.label}</h2>
